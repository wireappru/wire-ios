...
@objc public final class ConversationNameCell: IconSystemCell {
    ...
    private func createConstraints() {
        constrain(messageContentView, labelView, nameLabel) { container, label, nameLabel in
            ...
            nameLabel.top == label.bottom + 3
            nameLabel.bottom == container.bottom
        }
    }
    ...
    private func updateLabel() {
        guard let systemMessage = message.systemMessageData,
            systemMessage.systemMessageType == .newConversationWithName else { return }
    ...
    private func attributedTitle(for message: ZMConversationMessage) -> NSAttributedString? {
        ...
        let title = key(with: "title").localized(pov: sender.pov, args: senderString) && labelFont && labelTextColor
        return title
    }
    ...
    private func key(with pathComponent: String) -> String {
        return "content.system.conversation.with_name.\(pathComponent)"
    }
